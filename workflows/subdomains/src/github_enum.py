...
def github_enum(domains_file, output_path, notify_enabled=False):
    ...
    try:
        ...
        # 2. Run github-subdomains for each domain
        for domain in domains:
            logger.info(f"Enumerating subdomains via GitHub for: {domain}")
            # Create a per-domain temporary file as the tool requires -o
            with tempfile.NamedTemporaryFile(mode='w+', delete=True) as domain_tmp:
                try:
                    # github-subdomains -d domain -k -o output
                    cmd = ["github-subdomains", "-d", domain, "-k", "-o", domain_tmp.name]
                    # check=False because github-subdomains might return non-zero if no results found
                    subprocess.run(cmd, capture_output=True, text=True, check=False)
                    
                    # Read back results if the file was written to
                    if os.path.exists(domain_tmp.name) and os.path.getsize(domain_tmp.name) > 0:
                        domain_tmp.seek(0)
                        for line in domain_tmp:
                            # Sanitize input (equivalent to sed 's/\*\.//g; s/\*//g')
                            clean_line = line.replace("*.", "").replace("*", "").strip()
                            if clean_line:
                                found_subs.add(clean_line)
                except Exception as e:
                    logger.error(f"Error running github-subdomains for {domain}: {e}")
        ...
        if newly_discovered:
            ...
            logger.info(f"Discovered {len(newly_discovered)} new subdomains via GitHub.")
            ...
            if notify_enabled and not first_run:
                ...
                if shutil.which("notify") and os.path.isfile(notify_config_path):
                    ...
                    notify_msg = f"[{timestamp}] New Subdomains (GitHub)\n\n" + "\n".join(newly_discovered)
                    try:
                        subprocess.run(
            ...
            # Update the master file using 'anew' if available
        ...
        # Update the latest github-run cache
...
if __name__ == "__main__":
    import sys
    # Usage: python github_enum.py <domains_file> <output_path> [notify_enabled(true/false)]
    if len(sys.argv) < 3:
        print("Usage: python github_enum.py <domains_file> <output_path> [notify_enabled(true/false)]")
        sys.exit(1)
    ...
    github_enum(file_arg, out_arg, notify_arg)